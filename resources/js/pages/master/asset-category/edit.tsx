...
type AssetCategoryProps = {
    ...
    asset_coa_id: number | null;
    accumulation_coa_id: number | null;
    depreciation_coa_id: number | null;
    is_active: number;
};
...
export default function AssetCategoryEditScreen({
    category,
    coas,
}: {
    category: AssetCategoryProps;
    coas: CoaOption[];
}) {
    ...
    const { data, setData, put, processing, errors } =
        useForm<AssetCategoryFormData>({
            code: category.code ?? '',
            name: category.name ?? '',
            useful_life_in_years: category.useful_life_in_years ?? 0,
            useful_life_in_months: category.useful_life_in_months ?? 0,
            asset_coa_id: category.asset_coa_id
                ? String(category.asset_coa_id)
                : '',
            accumulation_coa_id: category.accumulation_coa_id
                ? String(category.accumulation_coa_id)
                : '',
            depreciation_coa_id: category.depreciation_coa_id
                ? String(category.depreciation_coa_id)
                : '',
    ...
    const submit: FormEventHandler<HTMLFormElement> = (e) => {
        ...
        put(assetCategoryData.update(category.id).url, {
            preserveScroll: true,
            onSuccess: () => {
                toast.success('Berhasil', {
                    description: 'Kategori aset berhasil diperbarui.',
                });
            },
            onError: () => {
                toast.error('Gagal', {
                    description:
                        'Terjadi kesalahan saat memperbarui kategori aset.',
                });
            },
        });
    };
    ...
    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            <Head title="Perbarui kategori aset" />

            <div className="px-5 py-6">
                <Heading
                    title="Perbarui Kategori Aset"
                    description="Perbarui data kategori aset"
                />
                ...
                <form onSubmit={submit} className="space-y-8 xl:px-12">
                    <div className="flex flex-col lg:flex-row lg:space-x-12">
                        <aside className="2xl:w-md w-full max-w-xl lg:w-[250px] xl:w-[350px]">
                            <HeadingSmall
                                title="Data Umum"
                                description="Perbarui kode, nama, masa manfaat, dan status"
                            />
                        </aside>
