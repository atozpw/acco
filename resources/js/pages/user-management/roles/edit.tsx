...
export default function RoleEditScreen({
    ...
    const initialGuardRef = useRef(data.guard_name);
    ...
    useEffect(() => {
        if (data.guard_name !== initialGuardRef.current) {
            setData('permissions', []);
            // eslint-disable-next-line react-hooks/set-state-in-effect
            setPermissionSearch('');
        }
    }, [data.guard_name, setData]);
    ...
    const submit: FormEventHandler<HTMLFormElement> = (e) => {
        ...
        put(roleRoute.update(role.id).url, {
            preserveScroll: true,
            onSuccess: () => {
    ...
    return (
        <AppLayout breadcrumbs={buildBreadcrumbs(roleRoute.index().url)}>
            <Head title="Perbarui role" />

            <div className="px-5 py-6">
                <Heading
                    title="Perbarui Role"
                    description="Ubah nama, guard, atau permissions role"
                />
