...
interface SalesDeliveryPayload {
    id: number;
    reference_no: string;
...
export default function SalesDeliveryShowScreen({
    delivery,
}: {
    delivery: SalesDeliveryPayload;
}) {
    const details = delivery.details ?? [];
    const contactName = delivery.contact?.name ?? '-';
    const contactAddress = delivery.contact?.address?.trim() || '-';
    const pageTitle = delivery.reference_no
        ? `Detail Pengiriman ${delivery.reference_no}`
        : 'Detail Pengiriman';

    const generalInfo = [
        { label: 'Nomor Pengiriman', value: delivery.reference_no },
        { label: 'Tanggal Pengiriman', value: delivery.formatted_date ?? '-' },
    ];
            ...
            <div className="px-5 py-6">
                <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                    <Heading
                        ...
                        description="Tinjau detail transaksi pengiriman barang"
                    />
                    <div className="flex gap-3">
                        <Button asChild variant="outline">
                            <Link href={salesDelivery.index().url}>
                                <Undo2 />
                                Kembali
                            </Link>
                        </Button>
                        <Button asChild>
                            <Link href={salesDelivery.edit(delivery.id).url}>
                                <Settings2 />
                                Perbarui
                            </Link>
                        </Button>
