...
const twoDecimals = (value: string | number | null | undefined) =>
    toNumber(value).toFixed(2);

export default function SalesDeliveryEditScreen({
    delivery,
    contacts,
    warehouses,
    products,
    ...
    const initialDetails = useMemo<DetailForm[]>(
        () =>
            delivery.details.map((detail) => ({
                product_id: String(detail.product_id),
                qty: twoDecimals(detail.qty),
                price: twoDecimals(detail.price),
                amount: twoDecimals(detail.amount),
                tax_amount: twoDecimals(detail.tax_amount),
                discount_percent: twoDecimals(detail.discount_percent),
                discount_amount: twoDecimals(detail.discount_amount),
                total: twoDecimals(detail.total),
                note: detail.note ?? '',
                ...
                discount_type:
                    Number(detail.discount_percent) > 0 ? 'percent' : 'amount',
            })),
        [delivery.details],
    );

    const { data, setData, put, processing, errors } = useForm<FormData>({
        contact_id: String(delivery.contact_id),
        warehouse_id: String(delivery.warehouse_id),
        department_id: String(delivery.department_id),
        project_id: String(delivery.project_id),
        reference_no: delivery.reference_no,
        date: delivery.date,
        description: delivery.description,
        amount: twoDecimals(delivery.amount),
        tax_amount: twoDecimals(delivery.tax_amount),
        discount_percent: twoDecimals(delivery.discount_percent),
        discount_amount: twoDecimals(delivery.discount_amount),
        total: twoDecimals(delivery.total),
        is_closed: Boolean(delivery.is_closed),
        details: initialDetails,
    });

    const [formattedDetailQty, setFormattedDetailQty] = useState<string[]>(
        initialDetails.map((d) => formatLocal(d.qty)),
    );
    const [formattedDetailPrice, setFormattedDetailPrice] = useState<string[]>(
        initialDetails.map((d) => formatLocal(d.price)),
    );
    const [formattedDetailDiscountPercent, setFormattedDetailDiscountPercent] =
        useState<string[]>(
            initialDetails.map((d) => formatLocal(d.discount_percent)),
        );
    const [formattedDetailDiscountAmount, setFormattedDetailDiscountAmount] =
        useState<string[]>(
            initialDetails.map((d) => formatLocal(d.discount_amount)),
        );
    ...
    const addRow = () => {
        setData((prev) => {
            const nextDetails = [...prev.details, { ...blankDetail }];
    ...
    const handleSubmit: FormEventHandler<HTMLFormElement> = (e) => {
        ...
        put(salesDelivery.update(delivery.id).url, {
            preserveScroll: true,
            onSuccess: () => {
    ...
    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            ...
            <div className="px-5 py-6">
                <Heading
                    title="Ubah Pengiriman Barang"
                    description="Perbarui transaksi pengiriman barang"
                />
                ...
                <form onSubmit={handleSubmit} className="space-y-8 xl:px-12">
                    <div className="flex flex-col lg:flex-row lg:space-x-12">
                        <aside className="w-full max-w-xl lg:w-[260px] xl:w-[320px]">
                            <HeadingSmall
                                title="Data Umum"
                                description="Perbarui informasi utama pengiriman"
                            />
                        </aside>
                    ...
                    <div className="flex flex-col space-y-6">
                        <aside className="w-full">
                            <HeadingSmall
                                title="Detail Barang"
                                description="Atur item yang dikirim"
                            />
                        </aside>
                    ...
                    <div className="mt-6 flex items-center justify-end">
                        <Button
                            ...
                            className="mr-3"
                        >
                            <Link href={salesDelivery.index().url}>Batal</Link>
                        </Button>
