...
export default function ReceivablePaymentCreateScreen({
    referenceNumber,
    contacts,
    coas,
    departments,
    ...
    const handleContactChange = (value: string) => {
        setData((prev) => {
            ...
            const nextDetails = prev.details.map((detail) => {
                if (!detail.sales_invoice_id) return detail;
                if (!allowedInvoices.has(detail.sales_invoice_id)) {
                    return {
                        ...detail,
                        sales_invoice_id: '',
                        amount: formatLocalNumber(0),
    ...
    const handleSubmit: FormEventHandler<HTMLFormElement> = (e) => {
        ...
        transform((form) => {
            const detailPayload = form.details
                .filter((detail) => detail.sales_invoice_id)
                .map((detail) => ({
                    sales_invoice_id: Number(detail.sales_invoice_id),
        ...
        post(receivablePayment.store().url, {
            preserveScroll: true,
            onSuccess: () => {
                toast.success('Berhasil', {
                    description: 'Pembayaran piutang berhasil dibuat.',
                });
            },
            onError: () => {
                toast.error('Gagal', {
    ...
    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            <Head title="Buat Pembayaran Piutang" />

            <div className="px-5 py-6">
                <Heading
                    title="Tambah Pembayaran Piutang"
                    description="Catat penerimaan pembayaran piutang usaha"
                />
                    ...
                    <div className="flex flex-col space-y-6">
                        <aside className="w-full">
                            <HeadingSmall
                                ...
                                description="Hubungkan faktur dan alokasikan nominalnya"
                            />
                        </aside>
                                                ...
                                                <Fragment key={index}>
                                                    <tr className="border-t">
                                                        ...
                                                        <td className="px-4 py-2 align-top">
                                                            <InputCombobox
                                                                name={`details.${index}.sales_invoice_id`}
                                                                items={
                                                                    invoiceItems
                                                                }
                                                                placeholder={
