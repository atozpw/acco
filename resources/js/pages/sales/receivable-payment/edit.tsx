...
export default function ReceivablePaymentEditScreen({
    payment,
    contacts,
    coas,
    departments,
    ...
    const formattedDetails = payment.details.length
        ? payment.details.map((detail) => ({
              sales_invoice_id: detail.sales_invoice_id
                  ? String(detail.sales_invoice_id)
                  : '',
    ...
    const handleSubmit: FormEventHandler<HTMLFormElement> = (e) => {
        ...
        put(receivablePayment.update(payment.id).url, {
            preserveScroll: true,
            onSuccess: () => {
                toast.success('Berhasil', {
                    description: 'Pembayaran piutang berhasil diperbarui.',
                });
            },
            onError: () => {
                toast.error('Gagal', {
                ...
                <form onSubmit={handleSubmit} className="space-y-8 xl:px-12">
                    <div className="flex flex-col lg:flex-row lg:space-x-12">
                        <aside className="w-full max-w-xl lg:w-[260px] xl:w-[320px]">
                            <HeadingSmall
                                title="Data Umum"
                                description="Perbarui informasi utama pembayaran"
                            />
                        </aside>
