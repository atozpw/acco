...
type InvoicePayload = {
    ...
    deliveries: InvoiceDeliveryLink[];
};
...
export default function SalesInvoiceEditScreen({
    invoice,
    referenceCoa,
    contacts,
    coas,
    ...
    const deliveryMap = useMemo(() => {
        ...
        deliveries.forEach((deliveryOption) => {
            map[String(deliveryOption.id)] = deliveryOption;
        });
        return map;
    }, [deliveries]);
    ...
    const initialDeliveryLinks: DeliveryLinkForm[] = useMemo(() => {
        const mapped = invoice.deliveries.map((dl) => ({
            sales_delivery_id: String(dl.sales_delivery_id),
            note: dl.note ?? '',
        }));
        return mapped.length ? mapped : [{ sales_delivery_id: '', note: '' }];
    }, [invoice.deliveries]);
    ...
    const { data, setData, put, processing, errors, transform } =
        useForm<FormData>({
            ...
            is_delivery: Boolean(invoice.is_delivery),
            details: initialDetailsFromInvoice.length
                ? initialDetailsFromInvoice
                : [{ ...initialDetail }],
            deliveries: initialDeliveryLinks.length
                ? initialDeliveryLinks
                : [{ sales_delivery_id: '', note: '' }],
        });

    const [formattedDetailQty, setFormattedDetailQty] = useState<string[]>(
        (initialDetailsFromInvoice.length
            ? initialDetailsFromInvoice
            : [initialDetail]
        ).map((d) => formatLocal(d.qty)),
    );
    const [formattedDetailPrice, setFormattedDetailPrice] = useState<string[]>(
        (initialDetailsFromInvoice.length
            ? initialDetailsFromInvoice
            : [initialDetail]
        ).map((d) => formatLocal(d.price)),
    );
    const [formattedDetailDiscountPercent, setFormattedDetailDiscountPercent] =
        useState<string[]>(
            (initialDetailsFromInvoice.length
                ? initialDetailsFromInvoice
                : [initialDetail]
            ).map((d) => formatLocal(d.discount_percent)),
        );
    const [formattedDetailDiscountAmount, setFormattedDetailDiscountAmount] =
        useState<string[]>(
            (initialDetailsFromInvoice.length
                ? initialDetailsFromInvoice
                : [initialDetail]
            ).map((d) => formatLocal(d.discount_amount)),
        );
    ...
    const resetByDeliveryToggle = (checked: boolean) => {
        ...
        const baseDeliveries = [{ sales_delivery_id: '', note: '' }];
    ...
    const handleSubmit: FormEventHandler<HTMLFormElement> = (e) => {
        ...
        put(salesInvoice.update(invoice.id).url, {
            preserveScroll: true,
            onSuccess: () => {
