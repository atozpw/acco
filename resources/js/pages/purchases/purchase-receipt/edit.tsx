...
const toDetailForm = (detail: ReceiptDetailPayload): DetailForm => ({
    product_id: detail.product_id ?? '',
    qty: detail.qty ?? '0.00',
    price: detail.price ?? '0.00',
    amount: detail.amount ?? '0.00',
    tax_amount: detail.tax_amount ?? '0.00',
    discount_percent: detail.discount_percent ?? '0.00',
    discount_amount: detail.discount_amount ?? '0.00',
...
export default function PurchaseReceiptEditScreen({
    receipt,
    contacts,
    warehouses,
    products,
    ...
    const normalizedDetails = receipt.details.length
        ? receipt.details.map(toDetailForm)
        : [{ ...initialDetail }];
    ...
    const handleSubmit: FormEventHandler<HTMLFormElement> = (e) => {
        ...
        put(purchaseReceipt.update(receipt.id).url, {
            preserveScroll: true,
            onSuccess: () => {
            ...
            onError: () => {
                toast.error('Gagal', {
                    description:
                        'Terjadi kesalahan saat memperbarui penerimaan.',
                });
            },
        });
    };
    ...
    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            <Head title="Perbarui Penerimaan Barang" />
                ...
                <form onSubmit={handleSubmit} className="space-y-8 xl:px-12">
                    <div className="flex flex-col lg:flex-row lg:space-x-12">
                        <aside className="w-full max-w-xl lg:w-[260px] xl:w-[320px]">
                            <HeadingSmall
                                title="Data Umum"
                                description="Perbarui informasi utama penerimaan"
                            />
                        </aside>
