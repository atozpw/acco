...
const resolveDiscountPercent = (detail: InvoiceDetailPayload): number => {
    const discountPercent = parseNumber(detail.discount_percent);
    if (discountPercent > 0) return discountPercent;

    const discountAmount = parseNumber(detail.discount_amount);
    const amountValue = parseNumber(detail.amount);

    if (discountAmount > 0 && amountValue > 0) {
        return (discountAmount / amountValue) * 100;
    }

    return discountPercent;
};

export default function PurchaseInvoiceShowScreen({
    invoice,
}: {
            ...
            <div className="px-5 py-6">
                <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                    <Heading
                        ...
                        description="Tinjau detail data invoice pembelian"
                    />
                    <div className="flex gap-3">
                        ...
                        <Button asChild>
                            <Link href={purchaseInvoice.edit(invoice.id).url}>
                                <Settings2 />
                                Perbarui
                            </Link>
                        </Button>
