...
import AccountPayableFilterDialog, {
    type PayableFilterValues,
} from './partials/filter-dialog';

const breadcrumbs: BreadcrumbItem[] = [
    { title: 'Pembelian', href: purchases.index().url },
    { title: 'Daftar Utang Usaha', href: accountPayable.index().url },
    { title: 'Detail', href: '' },
];
...
type InvoiceDetail = {
    ...
    aging: AgingBuckets;
    details?: PayableDetailsPayload;
};

type PayableDetailPayment = {
    id: number;
...
export default function AccountPayableShowPage({
    contact_id,
    invoice,
    filters,
}: PageProps) {
    ...
    const detailSource = useMemo<PayableDetailsPayload>(() => {
    ...
    const selectedDetailPayments: PayableDetailPayment[] = isDetailReady
    ...
    useEffect(() => {
        ...
        if (
            ...
            router.get(
                accountPayable.show(contact_id),
                {
                    ...baseFilterQuery,
    ...
    const filterValues = useMemo<PayableFilterValues>(
        () => ({
    ...
    const handleApplyFilters = (values: PayableFilterValues) => {
        const query: Record<string, string | number> = {
            perPage: Number(itemsPage),
        };
        ...
        router.get(accountPayable.show(contact_id), query, {
            preserveScroll: true,
            preserveState: true,
        });
    };
    ...
    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            <Head title="Detail Utang Usaha" />

            <div className="px-5 py-6">
                <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                    <Heading
                        title="Detail Utang Usaha"
                        description="Menampilkan informasi rinci utang usaha"
                    />
                    <div className="flex gap-3">
                        <Button variant="outline" asChild>
                            <Link href={accountPayable.index().url}>
                                <Undo2 /> Kembali
                            </Link>
                        </Button>
                    </div>
                ...
                <div className="space-y-6">
                                ...
                                {invoice.data.map((row) => (
                                    <TableRow key={row.id}>
                                        ...
                                        <TableCell className="text-right align-baseline">
                                            {formatCurrency(row.aging.lt_30)}
                                        </TableCell>
                                        <TableCell className="text-right align-baseline">
                                            {formatCurrency(
                                                row.aging.between_30_60,
                                            )}
                                        </TableCell>
                                        <TableCell className="text-right align-baseline">
                                            {formatCurrency(
                                                row.aging.between_60_90,
                                            )}
                                        </TableCell>
                                        <TableCell className="pe-4 text-right align-baseline">
                                            {formatCurrency(row.aging.gt_90)}
                                        </TableCell>
                                        <TableCell className="text-right align-baseline">
                                            <Button
                                                type="button"
                    ...
                    {shouldShowDetails && (
                        <div className="space-y-4">
                            <HeadingSmall
                                title="Rincian Utang"
                                description="Linimasa pergerakan utang"
                            />
                            <div className="overflow-hidden rounded-md border">
                                <Table>
                                    <TableHeader>
