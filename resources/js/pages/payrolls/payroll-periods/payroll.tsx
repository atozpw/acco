...
export default function PayrollShowScreen({ payroll }: Props) {
    ...
    const periodReferenceNo = payroll.periode?.reference_no ?? '-';
    const categoryName = payroll.periode?.category?.name ?? '-';
    const periodDate =
        payroll.periode?.formatted_date || payroll.periode?.date || '-';
    const periodRange =
        payroll.periode?.formatted_start_date ||
        payroll.periode?.formatted_end_date
            ? `${payroll.periode?.formatted_start_date || '-'} s.d. ${payroll.periode?.formatted_end_date || '-'}`
            : '-';
    ...
    const breadcrumbs: BreadcrumbItem[] = useMemo(
        () => [
            ...
            {
                title: payroll.periode?.reference_no
                    ? payroll.periode.reference_no
                    : 'Detail Periode',
                href: payroll.payroll_periode_id
                    ? payrollPeriods.show(payroll.payroll_periode_id).url
                    : payrollPeriods.index().url,
            },
            { title: 'Detail Payroll', href: '' },
        ],
        [payroll.payroll_periode_id, payroll.periode?.reference_no],
    );

    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            <Head title={`Detail Payroll ${contactName}`} />
            ...
            <div className="px-5 py-6">
                ...
                <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                    <Heading
                        ...
                        description="Tinjau rincian komponen gaji karyawan"
                    />
                    <div className="flex gap-3">
                        <Button asChild variant="outline">
                            <Link
                                href={
                                    payroll.payroll_periode_id
                                        ? payrollPeriods.show(
                                              payroll.payroll_periode_id,
                                          ).url
                                        : payrollPeriods.index().url
                ...
                <div className="space-y-8">
                    ...
                    <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-4">
                        ...
                        <div className="grid gap-1">
                            ...
                            <p className="text-sm font-medium">{periodDate}</p>
                        </div>
                        <div className="grid gap-1">
                            <span className="text-sm text-muted-foreground">
                                Rentang Periode
                            </span>
                            <p className="text-sm font-medium">{periodRange}</p>
                        </div>
                        <div className="grid gap-1">
                            <span className="text-sm text-muted-foreground">
                                Dibuat Oleh
