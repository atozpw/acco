...
type DetailPayload = {
    ...
    component?: PayrollComponentOption | null;
};
...
export default function PayrollFormulaEditScreen({
    formula,
    payrollCategories = [],
    contacts = [],
    departments = [],
    ...
    const componentTypeMap = useMemo(() => {
        const map: Record<string, 'earning' | 'deduction'> = {};
        payrollComponents.forEach((c) => {
            map[String(c.id)] = c.type;
        });
        return map;
    }, [payrollComponents]);

    const { data, setData, put, processing, errors, transform } = useForm({
        payroll_category_id: String(formula.payroll_category_id ?? ''),
        contact_id: String(formula.contact_id ?? ''),
        department_id: String(formula.department_id ?? ''),
        project_id: formula.project_id ? String(formula.project_id) : '',
    });

    const initialEarnings: DetailItem[] = useMemo(() => {
        const list = (formula.details ?? [])
            .filter((d) => {
                const type =
                    d.component?.type ??
                    componentTypeMap[String(d.payroll_component_id)];
                return type === 'earning';
            })
            .map((d) => {
                const amt = Number(d.amount) || 0;
                return {
                    payroll_component_id: String(d.payroll_component_id),
                    amount: amt,
                    formatted_amount: formatLocal(amt),
                };
            });
    ...
    }, [formula.details, componentTypeMap]);

    const initialDeductions: DetailItem[] = useMemo(() => {
        const list = (formula.details ?? [])
            .filter((d) => {
                const type =
                    d.component?.type ??
    ...
    const [earnings, setEarnings] = useState<DetailItem[]>(initialEarnings);
    const [deductions, setDeductions] =
        useState<DetailItem[]>(initialDeductions);
    ...
    const handleSubmit: FormEventHandler = (e) => {
        ...
        put(payrollFormulas.update(formula.id).url, {
            preserveScroll: true,
            onSuccess: () => {
            ...
            onError: (errs) => {
                ...
                toast.error('Gagal', {
                    description:
                        errorValues[0] ||
                        'Terjadi kesalahan saat memperbarui data perhitungan gaji.',
                });
            },
        });
    };
    ...
    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            <Head title="Perbarui Perhitungan" />

            <div className="px-5 py-6">
                <Heading
                    title="Edit Perhitungan"
                    description="Perbarui formula perhitungan gaji"
                />
